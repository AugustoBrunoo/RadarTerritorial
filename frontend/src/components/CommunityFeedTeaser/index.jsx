...
export default function CommunityFeedTeaser() {
  ...
  useEffect(() => {
    async function fetchRecentReportsCount() {
      try {
        ...
        if (!error && count !== null) {
          setRecentReportsCount(count);
        }
      } catch (err) {
        console.error("Erro ao buscar a contagem de relatos:", err);
      }
    }
        ...
        {recentReportsCount >= 10 && (
          <div className="bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800/60 p-3 rounded-2xl flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">
              {recentReportsCount} novos relatos nas últimas 24h
            </span>
          </div>
        )}
        <Link
