...
export default function DeleteOrgaoModal({ isOpen, orgaoName, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-sm rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden flex flex-col p-6 sm:p-8 text-center relative animate-in zoom-in-95 duration-300">
        ...
        <h3 className="font-black text-xl text-zinc-900 dark:text-white mb-2">Remover Órgão?</h3>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6 font-medium leading-relaxed">
          Você está prestes a remover <strong className="text-zinc-900 dark:text-zinc-200">{orgaoName}</strong>. Esta ação não poderá ser desfeita e pode afetar relatos vinculados.
        </p>

        <div className="flex flex-col gap-3 w-full">
          <button 
            onClick={onConfirm}
            className="w-full px-4 py-3 rounded-xl font-bold text-sm bg-red-600 hover:bg-red-700 text-white transition-colors shadow-sm shadow-red-500/20 cursor-pointer"
          >
