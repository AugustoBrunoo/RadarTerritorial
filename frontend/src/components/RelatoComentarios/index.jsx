    ...
    <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
      ...
      <div className="mb-6 relative">
        ...
        <button 
          ...
          className="absolute bottom-3 right-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl px-4 py-1.5 text-sm font-bold hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
        ...
        {isLoading ? (
          ...
          comments.map(c => (
            <div key={c.id} className="flex gap-3 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="flex-1 bg-zinc-50 dark:bg-zinc-950/50 p-4 rounded-3xl rounded-tl-sm border border-zinc-100 dark:border-zinc-800/80 shadow-sm">
                <div className="flex justify-between items-start mb-1.5 gap-2">
                  <span className="font-bold text-sm text-zinc-900 dark:text-zinc-200 truncate">{c.authorName}</span>
                ...
                <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed whitespace-pre-wrap">{c.texto}</p>
              </div>
            </div>
          ))
        )}
