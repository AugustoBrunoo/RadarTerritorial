...
export default function RelatoLinhaDoTempo({ report, dateStr, orgao }) {
  ...
  const isEmAnalise = report.status === 'em_analise' || report.status === 'em_execucao';
    ...
    <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 mt-8 shadow-sm">
      ...
      <div className="relative pl-6 sm:pl-8 border-l-2 border-zinc-200 dark:border-zinc-800 space-y-8">
        ...
        <div className="relative">
          <div className="absolute -left-[35px] sm:-left-[43px] top-0.5 h-6 w-6 rounded-full bg-emerald-500 border-4 border-white dark:border-zinc-900 flex items-center justify-center shadow-md shadow-emerald-500/20 text-white">
        ...
        <div className="relative">
          <div className={`absolute -left-[35px] sm:-left-[43px] top-0.5 h-6 w-6 rounded-full border-4 border-white dark:border-zinc-900 flex items-center justify-center ${
            orgao || !isPendente
              ? 'bg-blue-600 shadow-md shadow-blue-500/30 text-white'
              : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-400'
          }`}>
            {orgao || !isPendente ? <Building2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
          </div>

          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className={`text-base font-bold ${orgao || !isPendente ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-700 dark:text-zinc-300'}`}>
            ...
            {orgao ? (
              <div className="mt-3.5 bg-zinc-50 dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 sm:p-5">
                <div className="flex items-start gap-3.5">
                  <div className="bg-blue-100/80 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 p-2.5 rounded-xl shrink-0">
                ...
                {/* Resposta Oficial ou Mensagem de Status */}
                {report.resposta_orgao ? (
                  <div className="mt-4 pt-3.5 border-t border-zinc-200/70 dark:border-zinc-800/80">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-blue-600 dark:text-blue-400 mb-1">
                      ...
                      <span>Despacho / Resposta Oficial do Órgão:</span>
                    </div>
                    <p className="text-sm text-zinc-700 dark:text-zinc-300 italic bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-3 rounded-xl leading-relaxed">
                      "{report.resposta_orgao}"
                    </p>
                  </div>
                ) : (
                  <div className="mt-3 text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5 font-medium">
                    <Clock className="h-3.5 w-3.5 text-amber-500" />
                    <span>Demanda em fila de atendimento operacional no órgão.</span>
                  </div>
                )}
              </div>
            ) : (
        ...
        <div className={`relative ${!isResolvido ? 'opacity-60' : ''}`}>
          <div className={`absolute -left-[35px] sm:-left-[43px] top-0.5 h-6 w-6 rounded-full border-4 border-white dark:border-zinc-900 flex items-center justify-center ${
            isResolvido
              ? 'bg-emerald-500 shadow-md shadow-emerald-500/30 text-white'
              : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-400'
          }`}>
            {isResolvido ? <CheckCircle2 className="h-3 w-3" /> : <div className="h-1.5 w-1.5 rounded-full bg-zinc-400"></div>}
          </div>

          <div>
            <h3 className={`text-base font-bold ${isResolvido ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-700 dark:text-zinc-300'}`}>
              Resolução & Validação Comunitária
            </h3>
            <p className="text-zinc-500 text-sm mt-1 leading-relaxed">
              {isResolvido
                ? "Problema concluído pelo órgão responsável e verificado pela comunidade local."
                : "Após a conclusão das obras ou reparos pelo órgão, o status será atualizado e validado pelos moradores."}
            </p>
          </div>
        </div>
